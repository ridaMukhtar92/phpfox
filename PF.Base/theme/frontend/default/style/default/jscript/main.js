...
$Behavior.repositionCoverPhoto = function(){
	function repositionCoverPhoto(sModule, iId){
		...
		}).parent().parent().append('<div id="save_reposition_cover" class="btn btn-primary" onclick="$(\'.pages_header_cover img, .profiles_banner_bg .cover img\').draggable(\'destroy\');$(\'.profiles_banner\').removeClass(\'editing\');$(\'#save_reposition_cover\').remove();">' + oTranslations['user.save'] + '</div>');
	}
