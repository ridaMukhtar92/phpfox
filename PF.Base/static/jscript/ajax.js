	...
	$('.on_enter_submit').keydown(function(e) {
		if (e.which == 13) {
			...
			if (value == '') {
				...
				if (holder.length > 0) {
					holder.find('.mail_messages:first').prepend('<div id="js_ajax_compose_error_message"><div class="error_message">' + getPhrase('mail.can_not_send_empty_message') + '</div></div>');
					setTimeout(function() {
					...
					if (boxContent.length > 0) {
						...
						if (errorMessage) errorMessage.html('<div class="error_message">' + getPhrase('mail.can_not_send_empty_message') + '</div>');
					}
				}
				return false;
			}
