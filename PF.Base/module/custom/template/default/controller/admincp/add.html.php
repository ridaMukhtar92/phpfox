...
<div id="js_group_holder" style="display:none;">
	...
	<form method="post" action="{url link='admincp.custom.add'}" id="js_group_field" onsubmit="if ({$sGroupGetJsForm}) {literal}{$(this).ajaxCall('custom.addGroup');}{/literal} return false;">
		...
		<div class="table_clear">
			<input type="submit" value="{phrase var='custom.add_group'}" class="button btn-primary" />
			<input type="button" value="{phrase var='custom.cancel_uppercase'}" class="button btn-danger" id="js_cancel_new_group" />
		</div>
	</form>
</div>
{/if}
...
<!--			{* This next block is used as a template *}-->
			<div class="table_header">
				{if $bIsEdit}{_p var="Extra Values"}{else}{phrase var='custom.values'}{/if}:
			</div>
			...
			<div class="table_clear">
				<input type="submit" value="{if $bIsEdit}{phrase var='custom.update'}{else}{phrase var='custom.add'}{/if}" class="button btn-primary" />
			</div>
		</div>
