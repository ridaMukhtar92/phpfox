...
{/if}
	...
	<div class="table js_add_hidden" id="js_type_image" style="display:none;">
		...
		{/if}		
			<div id="js_ad_upload_banner"{if $bIsEdit} style="display:none;"{/if}>
				<input type="file" accept="image/*" name="image" size="30" />{if $bIsEdit} - <a href="#" onclick="$('#js_ad_upload_banner').hide(); $('#js_ad_banner').show(); return false;">{phrase var='ad.cancel'}</a>{/if}
				<div class="extra_info">
	...
	<div class="table_clear">
		{if isset($aForms.is_custom) && $aForms.is_custom == '2'}
		<input type="submit" value="{phrase var='ad.approve'}" class="button btn-primary" name="val[approve]"/>
		<input type="submit" value="{phrase var='ad.deny'}" class="button btn-danger" name="val[deny]" />
		{else}
