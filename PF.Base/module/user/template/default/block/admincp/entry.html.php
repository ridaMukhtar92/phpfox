	...
	<div class="table form-group">
		...
		<div class="table_right">
			...
			<div id="js_group_upload_icon"{if !empty($aForms.icon_ext)} style="display:none;"{/if}>
				<input type="file" accept="image/*" name="icon" size="30" />{if !empty($aForms.icon_ext)} - <a href="#" onclick="$('#js_group_upload_icon').hide(); $('#js_group_icon').show(); return false;">{phrase var='user.cancel'}</a>{/if}
