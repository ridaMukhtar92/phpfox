...
<div id="js_language_package_holder">
	...
	{/if}
		{if Phpfox_Locale::instance()->getLangId() != $aLanguage.language_id}<a href="#" onclick="$('#js_language_package_holder').hide(); $('#js_loading_language').html($.ajaxProcess('{phrase var='language.loading' phpfox_squote=true}')); $.ajaxCall('language.process', 'id={$aLanguage.language_id}'); return false;">{/if}{$aLanguage.title}{if Phpfox_Locale::instance()->getLangId() != $aLanguage.language_id}</a>{/if}
	</div>
	<div class="clear"></div>
	{/foreach}
	<div class="clear"></div>
