...
function tb_show(caption, url, thisObject, sForceMessage, bForceNoCilck, sType)
{
	...
	if(url.indexOf('TB_inline') != -1){
                ...
                var sMainUrl = url + '&do_delete=true';
								...
								if (!params['call'])
								{
									sHtml += ' <input class="btn" type="submit" value="' + getPhrase('core.yes') + '" class="button" /> ';
								}
								else
								{
									sHtml += ' <input class="btn" type="button" value="' + getPhrase('core.yes') + '" class="button" onclick="$(\'#js_inline_delete_id\').ajaxCall(\'' + params['call'] + '\', \'' + queryString + '\'); tb_remove();" /> ';
								}
								sHtml += ' <input class="btn btn-success" type="button" value="' + getPhrase('core.no') + '" class="button" onclick="tb_remove();" /> ';
								sHtml += '</div>';
								sHtml += '</div>';
								if (!params['call'])
								{
