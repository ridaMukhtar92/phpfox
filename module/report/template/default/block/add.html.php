...
<div class="">
	...
	<div class="p_top_8">
		...
		<div class="table">
			<div class="table_right">
				<input type="button" value="{phrase var='core.submit'}" class="button btn-block btn-danger" onclick="if ( ($('#js_report').val() != '' || $('#feedback').val() != '' ) && confirm('{phrase var='core.are_you_sure' phpfox_squote=true}')) {left_curly} $.ajaxCall('report.insert', 'id={$iItemId}&amp;type={$sType}&amp;report=' + $('#js_report').val() + '&feedback='+$('#feedback').val()); {right_curly}" />
			</div>
		</div>
