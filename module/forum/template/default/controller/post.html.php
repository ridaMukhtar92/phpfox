...
{/if}
	...
	<div class="table form-group">
      ...
      {if defined('PHPFOX_FORUM_REPLY_THREAD')}
        {editor id='text' placeholder='forum.your_reply_dot_dot_dot'}
      {else}
        {editor id='text' placeholder='forum.your_message_dot_dot_dot'}
      {/if}
			{if !isset($iForumId)}
			{literal}
			<script>
			...
			{if isset($iForumId) && $iForumId > 0 && Phpfox::isModule('poll') && Phpfox::getUserParam('poll.can_create_poll') && Phpfox::getUserParam('forum.can_add_poll_to_forum_thread')}
				...
				<div class="table form-group">
					...
					<div id="js_attach_poll"{if $bIsEdit && $aForms.poll_id > 0} style="display:none;"{/if}>
						<input type="button" name="poll" value="{phrase var='forum.attach_poll'}" class="button btn-primary" onclick="tb_show('{phrase var='forum.attach_poll'}', $.ajaxBox('poll.add', 'height=340&amp;width=550&amp;item_id={$iForumId}&amp;module_id=forum'));" />
					</div>
					</div>
				</div>
			{/if}
	...
	<div class="table_clear">
		<input type="submit" id="js_forum_form_submit_btn" value="{if isset($iEditId)}{phrase var='forum.update'}{else}{phrase var='forum.submit'}{/if}" class="button btn-primary" />
	</div>
