	...
	<tr class="checkRow{if is_int($iKey/2)} tr{else}{/if}">
		...
		<td class="t_center">
			<div class="js_item_is_active"{if !$aGroup.is_active} style="display:none;"{/if}>
				<a href="#?call=rss.updateGroupActivity&amp;id={$aGroup.group_id}&amp;active=0" class="js_item_active_link" title="{phrase var='rss.deactivate'}">{img theme='misc/bullet_green.png' alt=''}</a>
			</div>
			<div class="js_item_is_not_active"{if $aGroup.is_active} style="display:none;"{/if}>
				<a href="#?call=rss.updateGroupActivity&amp;id={$aGroup.group_id}&amp;active=1" class="js_item_active_link" title="{phrase var='rss.activate'}">{img theme='misc/bullet_red.png' alt=''}</a>
			</div>		
		</td>		
	</tr>
	{/foreach}
