	...
	<form method="post" action="#">
		...
		<div class="{if is_int($phpfox.iteration.comment/2)}row1{else}row2{/if}{if $phpfox.iteration.comment == 1} row_first{/if}" id="js_comment_{$aComment.comment_id}">
			...
			<div style="margin-left:65px;">
				...
				<input type="button" value="{phrase var='comment.approve'}" class="button btn-primary" onclick="$.ajaxCall('comment.moderate' , 'action=approve&amp;id={$aComment.comment_id}'); return false;" />
				<input type="button" value="{phrase var='comment.deny'}" class="button btn-danger" onclick="$.ajaxCall('comment.moderate' , 'action=deny&amp;id={$aComment.comment_id}'); return false;" />
			</div>
			<div class="clear"></div>		
		</div>
	</form>
