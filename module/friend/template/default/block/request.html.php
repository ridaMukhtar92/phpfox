...
{if $sError}
	...
	{elseif $sError == 'user_asked_already'}
		...
		<div class="p_4">
			...
			<div class="p_4">
				<input type="submit" onclick="$('#js_process_request').ajaxCall('friend.processRequest', 'type=yes&amp;user_id={$aUser.user_id}');" value="{phrase var='friend.yes'}" class="button btn-primary" />
				<input type="submit" onclick="$('#js_process_request').ajaxCall('friend.processRequest', 'type=no&amp;user_id={$aUser.user_id}');" value="{phrase var='friend.no'}" class="button btn-primary" />
			</div>
		</div>	
...
{else}
	...
	<div class="p_top_15">
		<div class="table_clear" id="container_submit_friend_request">
			<input type="submit" onclick="{if $bSuggestion}$('#js_friend_suggestion').hide(); $('#js_friend_suggestion_loader').show(); {/if}$('#js_process_image').show(); $('#js_process_request').ajaxCall('friend.addRequest');" value="{phrase var='friend.send_friend_request'}" class="button btn-primary" /> <span id="js_process_image" style="display:none;">{img theme='ajax/add.gif'}</span>
		</div>
	</div>
{/if}
</form>