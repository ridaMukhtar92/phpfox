...
$Behavior.activityFeedProcess = function() {
	...
	$('.comment-limit:not(.is_checked)').each(function() {
		...
		if (totalHidden) {
			...
			cHolder.hide();
			var viewMore = $('<a href="#" class="load_more_comments dont-unbind">'+ oTranslations['comment.view_previous_comments'] +'</a>');
	...
	$('.activity_feed_content_display:not(.is_built)').each(function() {
		...
		if (t.outerHeight() > 300) {
			...
			t.prepend('<a href="#" class="feed_show_more_content no_ajax" onclick="$(this).parent().css(\'max-height\', \'none\'); $(this).remove(); return false;"> ' + oTranslations['feed.show_more'] + '</a>');
		}
	});
