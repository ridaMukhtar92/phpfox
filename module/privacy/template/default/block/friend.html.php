...
{/if}
	...
	<div id="js_custom_list_actual_holder">
		<form method="post" action="#" onsubmit="return $Core.updateCustomList(this);">
			...
			<div class="p_top_4">
				<input type="submit" class="button btn-primary" value="{phrase var='privacy.save'}" /> <a href="#" onclick="$('#js_create_custom_friend_list_holder').show(); $('#js_custom_list_actual_holder').hide(); return false;">{phrase var='privacy.or_create_a_new_list'}</a>
			</div>
		</form>
	</div>
		...
		<div id="js_create_custom_friend_list">
			...
			<div style="margin-top:10px;">
				...
				<form method="post" action="#" onsubmit="return false;">
					...
					<div class="form-group">
						<input type="submit" value="{phrase var='friend.add'}" class="button btn-primary" onclick="if ($('#js_add_new_list').val() != '') $('#js_add_new_list').ajaxCall('friend.addList', 'custom=true'); " />
					</div>
				</form>		
			</div>
		</div>
